...
class ONNX_management():
    ...
    def create_node(self, new_node, first_node, node_iterating, model):
        ...
        if 'Add' not in new_parameters['name']:
            try:
                if 'MaxPool' in new_parameters['name']:
                    # for 2d MaxPools, same formula as for Convs only leave the 'ch_out' factor out
                    new_parameters['MACs'] = new_parameters['kernel_shape'][0] * new_parameters['kernel_shape'][1] * new_parameters['ch_in'] * new_parameters['output_dim'][0] * new_parameters['output_dim'][1]
                else:
